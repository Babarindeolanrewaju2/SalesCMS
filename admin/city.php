...
<div class="wrapper">
                ...
                <tbody>
                  <?php
                    ...
                    while($row = $query->fetch_assoc()){
                      $rid = $row['region_id'];
                      $sql1 = "SELECT * FROM region WHERE region_id = '$rid' ";
                      $query1 = $conn->query($sql1);
                      $row1 = $query1->fetch_assoc();
                      echo "
                        <tr>
                          <td>".$row1['region_name']."</td>
                          <td>".$row['city_name']."</td>
                          <td>
                            <button class='btn btn-success btn-sm edit btn-flat' data-id='".$row['city_id']."'><i class='fa fa-edit'></i> Edit</button>
                            <button class='btn btn-danger btn-sm delete btn-flat' data-id='".$row['city_id']."'><i class='fa fa-trash'></i> Delete</button>
                          </td>
                        </tr>
                      ";
                    }
  ...
  <?php include 'includes/city_modal.php'; ?>
</div>
